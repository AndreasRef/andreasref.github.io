...
function Population(m, num) {
  ...
  for (var i = 0; i < num; i++) {
    //if (floor(i/rows))
    this.population[i] = new Painting(new DNA(), rectWidth / 2 + (i % rows) * (rectWidth + spacing) + spacing*1, rectHeight / 2 + spacing + floor(i / rows) * (rectHeight + spacing));
  }
  ...
  this.displayFitness = function() {
    for (var i = 0; i < this.population.length; i++) {
      ...
      votes += this.population[i].fitness;
    }
  }
  ...
  this.reproduction = function() {
    ...
    if (this.population.length > 1) { //Keep reducing the population untill only one is left
      ...
      for (var i = 0; i < this.population.length; i++) {
        ...
        this.population[i] = new Painting(child, rectWidth / 2 + (i % rows) * (rectWidth + spacing) + spacing*1, rectHeight / 2 + spacing + floor(i / rows) * (rectHeight + spacing));
      }
      this.generations++;
