...
class App {
  ...
  async init() {
    ...
    this.initTrackers();
    this.demographics.init().catch(err => console.warn('[Demographics] CSV load failed:', err));
    this.setupToggles();
      ...
      ">
        ...
        <div style="padding:16px 20px 18px;">
          <div id="loading-status" style="margin-bottom:10px;font-size:11px;">
            Opening: spreadsheet-of-you.xlsx
          </div>
          <div style="
          ...
          <div id="loading-begin" style="display:none;margin-top:14px;text-align:center;">
          </div>
        </div>
      </div>
    `;
  ...
  _showSecurityDialog() {
    ...
    // Replace the entire dialog body content
    const body = dialog.querySelector('div:last-child');
    body.innerHTML = `
      <div style="display:flex;align-items:flex-start;margin-bottom:14px;">
        <span style="font-size:28px;margin-right:10px;line-height:1;color:#D4A017;">&#9888;</span>
        <div>
          <div style="font-weight:bold;margin-bottom:6px;">Security Warning</div>
          <div style="font-size:11px;line-height:1.4;">
            This workbook contains macros that access your camera
            and analyze your appearance in real time.
          </div>
        </div>
      </div>
  ...
  async initTrackers() {
    try {
      ...
      console.warn('[App] Tracker init failed — running without tracking:', err);
    }
  }
  ...
  loop(timestamp) {
    ...
    const { face, segMask, faceApiResult } = this.trackers.update(
      video, timestamp, this.segmentationEnabled, null
    );
    ...
    // --- Update fake streams ---
    const fakeVals = this.fakeStreams.update(timestamp, smoothed, this.metrics.faceDetected);
    ...
    const { newPerson, archived } = this.archive.update(
      timestamp, this.metrics.faceDetected, smoothed, fakeVals
    );
    if (newPerson) {
    ...
    const dataReady = this._loadingDismissed && performance.now() >= this._dataHoldUntil;
    if (!dataReady) {
    ...
    const demoResult = (this.demographics.ready && smoothed.age != null && smoothed.genderMale != null)
    ...
    if (imageData) {
      this.cellMapper.update(
        smoothed, this.lastFaceLandmarks, null,
        this.segMask, this.frameCount, imageData, timestamp,
        fakeVals, mockVals, demoResult
      );

      this.renderer.render(imageData, this.segMask, this.cellMapper.mergedCells);
    }
  ...
  logMetrics(fakeVals) {
    ...
    const exprKeys = ['exprNeutral', 'exprHappy', 'exprSad', 'exprAngry',
                      'exprSurprised', 'exprFearful', 'exprDisgusted'];
    let dominantExpr = '\u2014';
    let maxExpr = -1;
    ...
    const gender = v.genderMale != null
      ? (v.genderMale > 0.5 ? `M(${f(v.genderMale)})` : `F(${f(1 - v.genderMale)})`)
      : '\u2014';
  ...
  update(timestamp, realMetrics) {
    ...
    if (realMetrics && Object.keys(realMetrics).length) {
      ...
      for (const [k, label] of Object.entries(exprMap)) {
        if ((realMetrics[k] || 0) > bestV) { bestV = realMetrics[k] || 0; best = label; }
      }
      ...
      out.stress = clamp(blink * 0.4 + move * 0.3 + 0.15 + Math.sin(t * 0.3) * 0.08);
    ...
    out.trustworthy = clamp(0.62 + Math.sin(t * 0.22) * 0.10 + Math.sin(t * 0.41) * 0.06);
    out.deception   = clamp(0.18 + Math.sin(t * 0.19) * 0.08 + Math.sin(t * 0.53) * 0.05);

    if (timestamp - this.lastShuffle > this.shuffleInterval || this.lastShuffle === 0) {
      this.lastShuffle = timestamp;
      this.profession = pick(['designer', 'engineer', 'student', 'artist', 'manager', 'teacher', 'analyst', 'researcher']);
      this.income     = pick(['lower', 'lower-mid', 'middle', 'upper-mid', 'upper']);
      this.mood       = pick(['contemplative', 'distracted', 'curious', 'self-conscious', 'relaxed', 'performing']);
