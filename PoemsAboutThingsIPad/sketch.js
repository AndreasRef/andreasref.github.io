...
function draw() {
  ...
  if (showLatestPhoto) {
    ...
    fill(255);
    if (children.length == 0) text("Writing a poem about " + lastPrediction + "...", 10, 20);

  } else {
    tint(255);
    //Show current prediction
    fill(255);
    text("I think this is " + currentPrediction, 10, 20);
    
    push();
    textAlign(CENTER);
    text("Touch the screen to take a picture ", width/2, 8*height/9);
    pop();
...
function addPoem() {
  ...
  for (var i = 0; i < min(5, children.length); i++) {
    ...
    if (textWidth(resultStrings[i]) > width * 0.90) {
      pg.text(splitter(resultStrings[i]), 10, 50 + ((height - 50) / min(5, children.length)) * i);
    } else {
      pg.text(resultStrings[i], 10, 50 + ((height - 50) / min(5, children.length)) * i);
    }
  }
}
