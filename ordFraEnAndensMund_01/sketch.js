/*
To do:
Load rigtige tekster
Load en tilfældig fra start
Stop når teksten er færdig
Test det med webcam
...
let allTexts = [
  [//Dagbog
    "Kære dagbog\nI dag har jeg fået lov at blive hjemme fra skole pga lockdown. Dagen har været sådan lidt kedelig, så jeg har lavet noget tegninger. Jeg har lavet en tegning af mig selv, min lillebror og min ponny. Mine forældre har lige været ude at handle og har købt en masse godter. Jeg har spist noget af det. Jeg fik også lov til at gå på nettet, men jeg blev lidt ked af det, fordi alle de nyheder om den smitte, der er kommet. Nå, men nu skal jeg over og se, om der er noget godt at se på TV.",
    "Kære dagbog\nMin far er så dum altså. Han vil ikke lade mig komme ud af huset, selvom det ikke er forbudt at køre rundt. Og nu har jeg ikke set nogen andre mennesker i 14 timer. Hvordan kan det være okay at blive indespærret derhjemme? Det er ikke i orden. Jeg kan ikke lave noget. Ikke engang kigge på tv, for min mor ser den der kedelige kanal.\nDet eneste, jeg kan gøre, er at se ud af vinduet. Jeg kan se, at der er mange mennesker udenfor. Så hvorfor kan jeg ikke komme ud af huset?",
    "Kære dagbog\nMin mor snakker hele tiden med arbejdet over computeren og min far er også ved computeren. Jeg vil gerne ud og spille med mine venner, men det må vi ikke. Men jeg fandt på noget sjovt. Jeg lavede et stort vandløb i stuen. Det var ikke så nemt, og der kom vand ud over hele gulvet. Da min mor opdagede det, sagde hun, at det var ikke sjovt. Jeg skulle tørre op med en klud. Øv ikke?",
    "Kære dagbog\nJeg har været så sej i dag. Forældrene har siddet i kontor og skrevet på computeren, og jeg har siddet i sofaen og været sej. Jeg så film og spillede spil og så en masse film. Jeg fik også lavet lektier, og det var virkelig sejt.\nNu er jeg træt og skal snart i seng.\nSlut.",
    "Kære dagbog,\nJeg er så sur, fordi min mor og far skal arbejde hjemmefra. Jeg har ikke nogen at lege med, og i dag har jeg ikke rigtig noget at lave. Jeg kan ikke være ved computeren, fordi min mor er ved at lave noget derinde. Jeg kan ikke spille xbox, fordi min far er ved at lave noget derinde. Det eneste gode er, at min lillesøster er her. Hun vil ikke lege, men vi sidder alligevel sammen og spiller nogle spil og piller lidt med mine Lego-klodser. I morgen skal vi have det sjovt, men i dag er det bare lidt trist."],
  [//Dating
...
let personaCategories = ["dagbog", "dating", "kat", "netto", "teen"]
...
function preload() {
  for (let i = 0; i < n; i++) {
    ...
    personaCounters[i] = 0; //sloppy

    for (let j = 0; j < n; j++) {
      personasClass[i][j] = new Persona("imgs/" + personaCategories[i] + j + ".jpeg", "text: " + j, 30 + i * 120, sketchH - 100, allTexts[i][j]);
    }
  }
}
...
function setup() {
  ...
  ground = matter.makeBarrier(width / 2, height - 80, width, 50);
