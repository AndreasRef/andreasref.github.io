...
skeletonization.setup = function(width,height){
  ...
  skeletonization.thinningIterationSeq = function(im,iter,bbox){
    var marker = cv.Mat.zeros(im.size(), cv.CV_8UC1);
    var one = 255;
    for (var i = bbox[1]; i < bbox[3]; i++)
    {
        for (var j = bbox[0]; j < bbox[2]; j++)
        {
            var p2 = im.ucharAt(i-1, j);
            var p3 = im.ucharAt(i-1, j+1);
            var p4 = im.ucharAt(i, j+1);
            var p5 = im.ucharAt(i+1, j+1);
            var p6 = im.ucharAt(i+1, j);
            var p7 = im.ucharAt(i+1, j-1);
            var p8 = im.ucharAt(i, j-1);
            var p9 = im.ucharAt(i-1, j-1);

            var A  = (p2 == 0 && p3 == one) + (p3 == 0 && p4 == one) + 
                     (p4 == 0 && p5 == one) + (p5 == 0 && p6 == one) + 
                     (p6 == 0 && p7 == one) + (p7 == 0 && p8 == one) +
                     (p8 == 0 && p9 == one) + (p9 == 0 && p2 == one);
            var B  = p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9;
            var m1 = iter == 0 ? (p2 * p4 * p6) : (p2 * p4 * p8);
            var m2 = iter == 0 ? (p4 * p6 * p8) : (p2 * p6 * p8);

            if (A == 1 && (B >= 2*one && B <= 6*one) && m1 == 0 && m2 == 0){
                marker.data[i*im.cols+j] = 255;
            }
        }
    }
